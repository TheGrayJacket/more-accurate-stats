...
switch (_itemType select 0) do
{
	case "Equipment": 
	{
		switch (_itemType select 1) do
		{
			case "Headgear": 
			{		
				//// HELMET TRUE STATS START
				// First let's define some private variables that we'll need:
				_configEntry = configFile >> "CfgWeapons" >> _itemClassName >> "ItemInfo" >> "HitpointsProtectionInfo";
				
				// Typing in values manually to avoid trouble and make it work.
				//High Priority
				_headArmor = getNumber (_configEntry >> "Head" >> "armor");
				//Low Priority
				_faceArmor = getNumber (_configEntry >> "Face" >> "armor");
				//Calculate total armor value from all Hitpoints. Low priority is divided by 4, since should not outweigh High Priority Hitpoints (at least they are not ignored!)
				_armor = _headArmor + (_faceArmor / 4);
				// Do the same for passthrough, but this time only take high priority hitpoints
				_headPassthrough = getNumber (_configEntry >> "Head" >> "Passthrough");
				...
				_explosiveResistance = (_armor / (1 + _armor))^18;

				// multiply them by 100 to obtain percentage and round them
				_ballisticProtection = round (_ballisticProtection*100);
				_explosiveResistance = round (_explosiveResistance*100);
				//// HELMET TRUE STATS END

				_mass = getNumber(configFile >> "CfgWeapons" >> _itemClassName >> "ItemInfo" >> "mass");
				_stats pushBack
				[
					"BALLISTIC", 
					format["<t size='1' font='puristaMedium' align='right'>%1%2</t>", _ballisticProtection, "%"],
					linearConversion [0, 100, _ballisticProtection, 0, 1, true]
				];
				_stats pushBack
				[
					"EXPLOSIVE", 
					format["<t size='1' font='puristaMedium' align='right'>%1%2</t>", _explosiveResistance, "%"],
					linearConversion [0, 100, _explosiveResistance, 0, 1, true]
				];
				_stats pushBack
				[
					"MASS", 
			...
			case "Vest":
			{
				//// VEST TRUE STATS START
				// First let's define some private variables that we'll need:
				_configEntry = configFile >> "CfgWeapons" >> _itemClassName >> "ItemInfo" >> "HitpointsProtectionInfo";
				
				// Typing in values manually to avoid trouble and make it work.
				//High Priority
				_abdomenArmor = getNumber (_configEntry >> "Abdomen" >> "armor");
				_chestArmor = getNumber (_configEntry >> "Chest" >> "armor");
				_diaphragmArmor = getNumber (_configEntry >> "Diaphragm" >> "armor");
				//Low Priority
				_armsArmor = getNumber (_configEntry >> "arms" >> "armor");
				_neckArmor = getNumber (_configEntry >> "neck" >> "armor");
				_pelvisArmor = getNumber (_configEntry >> "pelvis" >> "armor");
				//Calculate total armor value from all Hitpoints. Low priority is divided by 6, since should not outweigh High Priority Hitpoints (at least they are not ignored!)
				_armor = ((_abdomenArmor + _chestArmor + _diaphragmArmor)/3)+((_armsArmor + _neckArmor + _pelvisArmor)/6);
				// Do the same for passthrough, but this time only take high priority hitpoints
				_abdomenPassthrough = getNumber (_configEntry >> "Abdomen" >> "Passthrough");
				_chestPassthrough = getNumber (_configEntry >> "Chest" >> "Passthrough");
				_diaphragmPassthrough = getNumber (_configEntry >> "Diaphragm" >> "Passthrough");
				
				_passthrough = ((_abdomenPassthrough + _chestPassthrough + _diaphragmPassthrough)/3);
				...
				[
					...
					linearConversion [0, 200, _capacity, 0, 1, true]
				];
				_stats pushBack
				[
					"MASS", 
